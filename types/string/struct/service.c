...
static int accept_struct(sd_bus_message *m, void *userdata, sd_bus_error *ret_error) {
	int r;
	// struct Student s1;
	int id;
	char *name;
	...
	r = sd_bus_message_read(m, "(is)", &s1.id, &s1.name);
	if (r < 0) {
...
static const sd_bus_vtable calculator_vtable[] = {
        ...
        SD_BUS_METHOD("Struct",   "(is)", "s", accept_struct,   SD_BUS_VTABLE_UNPRIVILEGED),
        SD_BUS_VTABLE_END
};
