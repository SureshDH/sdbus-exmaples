...
static int accept_astructa(sd_bus_message *m, void *userdata, sd_bus_error *ret_error) {
	...
	r = sd_bus_message_enter_container(m, 'a', "(isai(is(i)))");
	if (r < 0) {
		fprintf(stderr, "Failed enter array container: %s\n", strerror(-r));
	}
	for(i=0; i<5; i++) {
			r = sd_bus_message_enter_container(m, 'r', "isai(is(i))");
			if (r < 0) {
			...
			// Book related
			r = sd_bus_message_enter_container(m, 'r', "is(i)");
			if (r < 0) {
				fprintf(stderr, "Failed enter inner structure container: %s\n", strerror(-r));
			}
			r = sd_bus_message_read(m, "i", &s1[i].b.pages);
			if (r < 0) {
				fprintf(stderr, "Failed to parse pages param: %s\n", strerror(-r));
			}
			r = sd_bus_message_read(m, "s", &s1[i].b.bookName);
			if (r < 0) {
				fprintf(stderr, "Failed to parse bookName param: %s\n", strerror(-r));
			}


			// Auther related
			r = sd_bus_message_enter_container(m, 'r', "i");
			if (r < 0) {
				fprintf(stderr, "Failed enter inner inner structure container: %s\n", strerror(-r));
			}
			r = sd_bus_message_read(m, "i", &s1[i].b.at.aid);
			if (r < 0) {
				fprintf(stderr, "Failed to parse aid param: %s\n", strerror(-r));
			}
			// Auther closed
			r = sd_bus_message_exit_container(m);
			if (r < 0) {
				fprintf(stderr, "Failed to exit inner inner struct container: %s\n", strerror(-r));
			}
			...
			r = sd_bus_message_exit_container(m);
			if (r < 0) {
				fprintf(stderr, "Failed to exit inner struct container: %s\n", strerror(-r));
			}
...
static const sd_bus_vtable calculator_vtable[] = {
        ...
        SD_BUS_METHOD("Astructa",   "a(isai(is(i)))", "s", accept_astructa,   SD_BUS_VTABLE_UNPRIVILEGED),
        SD_BUS_VTABLE_END
};
