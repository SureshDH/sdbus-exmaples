...
static int accept_astructa(sd_bus_message *m, void *userdata, sd_bus_error *ret_error) {
	...
	r = sd_bus_message_enter_container(m, 'a', "(isai(i))");
	if (r < 0) {
		fprintf(stderr, "Failed enter array container: %s\n", strerror(-r));
	}
	for(i=0; i<5; i++) {
			r = sd_bus_message_enter_container(m, 'r', "isai(i)");
			if (r < 0) {
...
static const sd_bus_vtable calculator_vtable[] = {
        ...
        SD_BUS_METHOD("Astructa",   "a(isai(i))", "s", accept_astructa,   SD_BUS_VTABLE_UNPRIVILEGED),
        SD_BUS_VTABLE_END
};
