...
static int accept_sarray(sd_bus_message *m, void *userdata, sd_bus_error *ret_error) {
	int value1;
	int value2;
	char *value[2];
	int r;
	...
	r = sd_bus_message_read(m, "as", 2, &value[0], &value[1]);
	if (r < 0) {
		fprintf(stderr, "Failed to parse parameters: %s\n", strerror(-r));
	}
	printf("Recieved value1 is: %s\n", value[0]);
	printf("Recieved value2 is: %s\n", value[1]);
...
static const sd_bus_vtable calculator_vtable[] = {
        ...
        SD_BUS_METHOD("Sarray",   "as", "s", accept_sarray,   SD_BUS_VTABLE_UNPRIVILEGED),
        SD_BUS_VTABLE_END
};
